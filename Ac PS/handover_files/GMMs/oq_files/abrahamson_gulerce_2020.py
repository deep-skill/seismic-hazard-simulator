...
def _get_adjustment(fine_region, imt):
        ### finer region adjustment for SA slab events
        if fine_region=="antapaccay":

            list_periods = [0.01, 0.1, 0.5, 1., 2.]
            list_adjustments = [-0.1, 0., -0.2, -0.2, 0.]

        elif fine_region=="antamina":
            list_periods = [0.01, 0.1, 0.5, 1., 2.]
            list_adjustments = [-0.15, -0.15, -0.15, -0.1, 0.]

        adj = np.interp(imt.period,  list_periods, list_adjustments)
        return(adj)
...
class AbrahamsonGulerce2020SInter(GMPE):
    ...
    def __init__(self, region="GLO", ergodic=True, apply_usa_adjustment=False,
                 sigma_mu_epsilon=0.0,adjustment=False, fine_region = None, **kwargs):
    ...
    def compute(self, ctx: np.recarray, imts, mean, sig, tau, phi):
        ...
        for m, imt in enumerate(imts):
            ...
            if self.adjustment:
                mean[m]+= _get_adjustment(self.fine_region, imt)
...
class AG2020SInterBCHydro15(AbrahamsonGulerce2020SInter):
    """
    Implements the 2020 Subduction ground motion model of Abrahamson &
    Gulerce (2020) for subduction in-slab earthquakes
...
class AG2020SSlabBCHydro15(AbrahamsonGulerce2020SInter):
    """
    Implements the 2020 Subduction ground motion model of Abrahamson &
    Gulerce (2020) for subduction in-slab earthquakes
