...
def _get_adjustment(self, imt):
        ### finer region adjustment for SA slab events
        list_periods = [0.01, 0.1, 0.5, 1., 2.]
        list_adjsutments = [-0.1, 0., -0.2, -0.2, 0.]
        
        adj = np.interp(imt.period,  list_periods, list_adjsutments)
        return(adj)
...
class AbrahamsonGulerce2020SInter(GMPE):
    ...
    def __init__(self, region="GLO", ergodic=True, apply_usa_adjustment=False,
                 sigma_mu_epsilon=0.0,adjustment=False, **kwargs):
    ...
    def compute(self, ctx: np.recarray, imts, mean, sig, tau, phi):
        ...
        for m, imt in enumerate(imts):
            ...
            if self.adjustment:
                ...
                mean[m]+= _get_adjustment(self, imt)
    ...
    def compute(self, ctx: np.recarray, imts, mean, sig, tau, phi):
        ...
        for m, imt in enumerate(imts):
            ...
            if self.ergodic:
                ...
                # Set all elements of sig to 0.60
                sig[m] = 0.60
