...
def _update_ctx(gsim, ctx):
    ...
    """
    tmp_ztor =  np.array(ctx.ztor.tolist())
    idx = tmp_ztor > 20.
    tmp_ztor[idx] = 20.
    ...
    if gsim.estimate_width:
        ...
        ctx.width[mask] = np.minimum(
            ctx.width[mask], (ctx.zbot[mask] - tmp_ztor[mask]) /
    ...
    if gsim.estimate_hypo_depth:
        ...
        zbor = tmp_ztor + ctx.width * np.sin(np.radians(ctx.dip))

        # Equation 35 of Campbell & Bozorgnia 2014
        mask = zbor > tmp_ztor
        dz = np.zeros_like(tmp_ztor)
        dz[mask] = np.exp(
            np.minimum(
                fdz_m[mask] + fdz_d[mask],
                np.log(0.9 * (zbor[mask] - tmp_ztor[mask]))))
        ctx.hypo_depth = tmp_ztor + dz

def get_sigma_mu_adjustment(ctx, imt):
    # Check if magnitude is above or below 7.0
    above = ctx.mag >= 7.0
    below = ctx.mag < 7.0
    
    # Initialize sigma_mu array
    sigma_mu = np.zeros_like(ctx.mag)
    
    # Calculate sigma_mu for magnitudes above 7.0
    sigma_mu[above] = 0.0665 * (ctx.mag[above] - 7.0) + 0.072
    
    # Set sigma_mu to 0.072 for magnitudes below 7.0
    sigma_mu[below] = 0.072
    
    # Adjust sigma_mu for periods greater than or equal to 1.0
    if imt.period >= 1.0:
        sigma_mu += 0.02107 * np.log(imt.period)
    
    # Additional adjustment for normal faults
    nm = (ctx.rake >= -150.) & (ctx.rake <= -30.)
    sigma_mu[nm] += 0.034
...
class CampbellBozorgnia2014(GMPE):
    ...
    SJ = 0  # 1 for Japan
    """
        Instantiate setting the sigma_mu_epsilon
    """
    ...
    def compute(self, ctx: np.recarray, imts, mean, sig, tau, phi):
        ...
        for m, imt in enumerate(imts):
            ...
            if self.sigma_mu_epsilon:
                ...
                sigma_mu = get_sigma_mu_adjustment(ctx, imt)
