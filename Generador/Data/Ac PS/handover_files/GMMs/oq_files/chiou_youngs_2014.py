...
def _get_centered_ztor(ctx):
    ...
    tmp_ztor_ztor =  np.array(ctx.ztor.tolist())
    idx = tmp_ztor_ztor > 20.
    tmp_ztor_ztor[idx] = 20.
    return tmp_ztor_ztor - mean_ztor
...
def get_hanging_wall_term(C, ctx):
    ...
    tmp_ztor[ix] = 20.
    if np.any(idx):
        fdist = 1.0 - (np.sqrt(ctx.rjb[idx] ** 2. + tmp_ztor[idx] ** 2.) /
                       (ctx.rrup[idx] + 1.0))
